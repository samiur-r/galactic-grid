...
export function ISSTracker() {
  ...
  const issData = (data && !Array.isArray(data) && 'content' in data) ? data.content?.[0]?.text : null;
        ...
        {!loading && !error && parsedData && (
          <div className="flex items-center space-x-2 text-green-400">
            ...
            <span className="text-sm">
              Live data • Updated {parsedData.lastUpdated}
            </span>
          </div>
        )}
      </div>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-mono">
                  {parsedData.latitude.toFixed(4)}°
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">Longitude:</span>
                <span className="font-mono">
                  {parsedData.longitude.toFixed(4)}°
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="mt-4 bg-gray-800 p-4 rounded-lg">
          ...
          <div className="h-32 bg-gray-700 rounded-lg flex items-center justify-center text-gray-400">
            <div className="text-center">
              ...
              <p className="text-sm">
                ISS Position: {parsedData.latitude.toFixed(2)}°,{" "}
                {parsedData.longitude.toFixed(2)}°
              </p>

...
export class SpaceApiService {
  ...
  private mapSpaceXLaunchToMission(spacexData: any): Mission {
    return {
      id: spacexData.id,
      name: spacexData.name,
      description: spacexData.details || 'SpaceX mission',
      agency: 'SpaceX',
      status: spacexData.success === null ? 'upcoming' : spacexData.success ? 'success' : 'failure',
      launch_date: spacexData.date_utc,
      mission_type: spacexData.payloads?.[0]?.type || 'Unknown',
      rocket: spacexData.rocket?.name || 'Falcon 9',
      payload: spacexData.payloads?.map((p: any) => p.name).join(', ') || 'Unknown',
      live_stream_url: spacexData.links?.webcast
    }
  }

  private mapLaunchLibraryToMission(llData: any): Mission {
    return {
      id: llData.id.toString(),
      name: llData.name,
      description: llData.mission?.description || llData.name,
      agency: llData.launch_service_provider?.name || 'Unknown',
      status: this.mapLaunchLibraryStatus(llData.status?.name),
  ...
  private mapLaunchLibraryToLaunch(llData: any): Launch {
    ...
    return {
      id: llData.id.toString(),
      mission_id: llData.mission?.id?.toString() || llData.id.toString(),
      name: llData.name,
      agency: llData.launch_service_provider?.name || 'Unknown',
  ...
  async getMissionDetails(missionId: string): Promise<Mission> {
    try {
      ...
      const spacexResponse = await fetch(`${this.config.SPACEX_API_URL}/launches/${missionId}`)
      ...
      const llResponse = await fetch(`${this.config.LAUNCH_LIBRARY_API_URL}/launch/${missionId}/`)
      ...
      console.warn(`Failed to fetch mission ${missionId}, using fallback:`, error)
      return {
        id: missionId,
  ...
  async searchMissions(params: MissionSearch): Promise<Mission[]> {
    try {
      const { limit = 20, offset = 0, agency, status, query, start_date, end_date } = params
      ...
      if (agency) llUrl += `&launch_service_provider__name=${encodeURIComponent(agency)}`
      ...
      if (llResponse.ok) {
        const llData = await llResponse.json()
        const llMissions = llData.results?.map((launch: any) => this.mapLaunchLibraryToMission(launch)) || []
        missions.push(...llMissions)
      }
      
      // Also try SpaceX API for SpaceX missions
      if (!agency || agency === 'SpaceX') {
        const spacexUrl = `${this.config.SPACEX_API_URL}/launches?limit=${Math.min(limit, 100)}`
        const spacexResponse = await fetch(spacexUrl)
        
        if (spacexResponse.ok) {
          const spacexData = await spacexResponse.json()
          let spacexMissions = spacexData.map((launch: any) => this.mapSpaceXLaunchToMission(launch))
          
          // Apply filters
          if (status) {
            spacexMissions = spacexMissions.filter((m: Mission) => m.status === status)
          }
          if (query) {
          ...
          if (start_date || end_date) {
            spacexMissions = spacexMissions.filter((m: Mission) => {
              ...
              if (start_date && launchDate < new Date(start_date)) return false
              if (end_date && launchDate > new Date(end_date)) return false
              return true
            })
          }
  ...
  async getUpcomingLaunches(params: LaunchSearch): Promise<Launch[]> {
    try {
      ...
      let url = `${this.config.LAUNCH_LIBRARY_API_URL}/launch/upcoming/?limit=${limit}&net__gte=${now.toISOString()}&net__lte=${futureDate.toISOString()}`
      ...
      return data.results?.map((launch: any) => this.mapLaunchLibraryToLaunch(launch)) || []
      ...
      return [{
        ...
        rocket: 'Falcon 9 Block 5',
        launch_date: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
        launch_time_utc: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
  ...
  async getSatelliteData(params: { satelliteId?: string; category?: 'active' | 'inactive' | 'debris'; limit?: number }): Promise<Satellite[]> {
    try {
      ...
      if (satelliteId) {
        ...
        if (this.config.N2YO_API_KEY) {
          try {
            const response = await fetch(`${this.config.N2YO_API_URL}/positions/${satelliteId}/0/0/0/1/?apiKey=${this.config.N2YO_API_KEY}`)
            if (response.ok) {
              const data = await response.json()
              if (data.positions?.[0]) {
                ...
                return [{
                  ...
                  visibility: pos.elevation > 0 ? 'visible' : 'eclipsed',
                  country: 'Unknown'
                }]
              }
            }
